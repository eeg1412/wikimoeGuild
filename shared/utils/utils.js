...
// 恶魔头像数量（server/publicgame/demon/ 下的 1.webp ~ N.webp）
const DEMON_AVATAR_COUNT = 27

/**
 * 随机获取恶魔头像ID（1 ~ DEMON_AVATAR_COUNT）
 * @param {Function} [rng=Math.random] - 随机数生成函数，可传入种子随机函数以保持确定性
 * @returns {number}
 */
export function getRandomDemonAvatarId(rng = Math.random) {
  return Math.floor(rng() * DEMON_AVATAR_COUNT) + 1
}

// 恶魔名字词库
// 前半部分
const DEMON_NAME_PREFIXES = [
  '幽影',
  '赤焰',
  '霜月',
  '青岚',
...
// 生成随机的恶魔名字
export function generateRandomDemonName() {
  const prefix =
    DEMON_NAME_PREFIXES[Math.floor(Math.random() * DEMON_NAME_PREFIXES.length)]
  const suffix =
