<template>
  <div>
    <!-- 页面标题 -->
    <div class="mb-4">
      <h2 class="text-xl font-semibold dark:text-gray-100">玩家登录日志</h2>
...
import {
  listGamePlayerLoginLogsApi,
  deleteGamePlayerLoginLogApi,
  batchDeleteGamePlayerLoginLogsApi
} from '@/api/admin/gamePlayerLoginLog.js'
...
async function fetchData() {
  ...
  try {
    ...
    const res = await listGamePlayerLoginLogsApi(params)
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm('确认删除该登录日志？', '提示', {
      type: 'warning'
    })
    deletingId.value = row._id
    await deleteGamePlayerLoginLogApi(row._id)
...
async function handleBatchDelete() {
  ...
  try {
    await ElMessageBox.confirm('确认按时间范围批量删除登录日志？', '警告', {
      type: 'warning'
    })
    ...
    const res = await batchDeleteGamePlayerLoginLogsApi(data)
