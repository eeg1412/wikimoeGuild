<template>
  <div>
    <!-- 页面标题 -->
    <div class="mb-4">
      <h2 class="text-xl font-semibold dark:text-gray-100">邮件验证码</h2>
    ...
    <el-card shadow="never">
      ...
      >
        ...
        <ResponsiveTableColumn
          ...
          align="center"
        />
        <ResponsiveTableColumn label="是否使用" width="90" align="center">
        ...
        <ResponsiveTableColumn label="发送时间" min-width="170">
          <template #default="{ row }">
...
import {
  listGameMailCodesApi,
  deleteGameMailCodeApi,
  batchDeleteGameMailCodesApi
} from '@/api/admin/gameMailCode.js'
...
const searchForm = reactive({ email: '', ip: '', success: '', used: '' })
...
async function fetchData() {
  ...
  try {
    ...
    if (searchForm.used !== '') params.used = searchForm.used
    const res = await listGameMailCodesApi(params)
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm(`确认删除该验证码记录？`, '提示', {
      type: 'warning'
    })
...
async function handleBatchDelete() {
  ...
  try {
    await ElMessageBox.confirm('确认按时间范围批量删除验证码记录？', '警告', {
      type: 'warning'
    })
